...
TEST_CASE("limit: loop too large") {
  std::string source{R"(
var a = 0;
while (false) {
)"};
  for (auto i = 0; i < UINT16_MAX / 2 - 3; ++i) {
    source += " nil;\n";
  }
  source += "}\n";
  const std::string expected{R"([line 32768] Error at '}': Loop body too large.
)"};
  REQUIRE_EQ(run(source), expected);
}

TEST_CASE("limit: no reuse constants") {
  std::string source{R"(
fun f() {
)"};
  ...
  const std::string expected{
      R"([line 260] Error at '1': Too many constants in one chunk.
)"};
  REQUIRE_EQ(run(source), expected);
}

TEST_CASE("limit: stack overflow") {
  std::string source{R"(
fun foo() {
)"};
  for (auto i = 1; i <= 16; ++i) {
    source += "var a" + std::to_string(i) + ";\n";
  }
  source += R"(
  foo();
}
foo();
  ...
  std::string expected{R"(Stack overflow.
)"};
  for (auto i = 0; i < 63; ++i) {
    expected += R"([line 0020] in <func: foo>
)";
  }
  expected += R"([line 0022] in <script>
)";
  REQUIRE_EQ(run(source), expected);
}

TEST_CASE("limit: too many constants") {
  std::string source{R"(
fun f() {
)"};
  for (auto i = 0; i <= UINT8_MAX; ++i) {
  ...
  const std::string expected{
      R"([line 260] Error at 'oops': Too many constants in one chunk.
)"};
  REQUIRE_EQ(run(source), expected);
}

TEST_CASE("limit: too many locals") {
  std::string source{R"(
fun f() {
  ...
  const std::string expected{
      R"([line 36] Error at 'oops': Too many local variables in function.
)"};
  REQUIRE_EQ(run(source), expected);
}

TEST_CASE("limit: too many upvalues") {
  std::string source{R"(
fun f() {
  ...
  const std::string expected{
      R"([line 70] Error at 'oops': Too many closure variables in function.
)"};
  REQUIRE_EQ(run(source), expected);
}
